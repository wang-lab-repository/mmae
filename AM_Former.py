...
from amformermodels.Attention.blocks import *
...
class FTTransformer(nn.Module):
    def __init__(
            self,
            dim, depth, heads, attn_dropout, ff_dropout, use_cls_token, groups, sum_num_per_group,
            prod_num_per_group, cluster, target_mode, token_descent, use_prod, num_special_tokens,
            categories, out, num_cont, num_cate, use_sigmoid, qk_relu
    ):
        super().__init__()
        '''
        dim: token dim
    ...
    def forward(self, x_categ, x_numer, step=0, return_attn=False):
